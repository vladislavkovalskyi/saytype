...
private struct ReadyTile: View {
    ...
    private func headline(info: WhisperModelInfo) -> String {
        switch state {
        case .ready: "Зажми \(model.settings.value.recordKey.title)"
...
private struct TextTile: View {
    ...
    var body: some View {
        ...
        Tile(title: "Текст", art: TileArt(name: "ObjectTextcard", width: 230, right: -30, bottom: -44), action: action) {
            TileValue(value: "Умная структура", detail: Text("списки и абзацы · Qwen 2B"))
            Toggle(isOn: $settings.value.smartStructure) { EmptyView() }
