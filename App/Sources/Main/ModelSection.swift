...
struct WhisperModelInfo: Equatable {
    ...
    var megabytes: Int { isTurbo ? 626 : 947 }
...
struct ModelSection: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .top, spacing: 16) {
                VStack(spacing: 0) {
                    ToggleRow("Умная структура", detail: "Qwen 2B · 1,2 ГБ", isOn: $settings.value.smartStructure, accent: world.accent)
                    RowDivider()
                ...
                MemoryPanel(info: info, smartStructure: settings.value.smartStructure)
...
private struct OtherModelsPanel: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            RowDivider(opacity: 0.16)
                .padding(.vertical, 18)

            Toggle(isOn: $settings.value.systemEngineFallback) {
                VStack(alignment: .leading, spacing: 1) {
                    Text("Системная диктовка").font(.onest(17, .semibold))
                    Text("без загрузки · запасной движок")
                        .font(.onest(13))
                        .foregroundStyle(.white.opacity(0.74))
                }
            }
            .toggleStyle(WorldToggleStyle(accent: world.accent))
        }
        .padding(.horizontal, 24)
        .padding(.vertical, 22)
    }
...
private struct MemoryPanel: View {
    ...
    var body: some View {
        let parts = [Part(name: info.shortTitle, gigabytes: info.memoryGB, opacity: 1)]
            + (smartStructure ? [Part(name: "Qwen 2B", gigabytes: 1.3, opacity: 0.62)] : [])
