...
enum CodeWords {
    ...
    static func isCode(_ word: String) -> Bool {
        let core = word.trimmingCharacters(in: edgePunctuation)
        guard core.count > 2, core.unicodeScalars.allSatisfy({ $0.isASCII && !CharacterSet.whitespacesAndNewlines.contains($0) }) else { return false }
        let scalars = Array(core.unicodeScalars)
        // useEffect, useUser
        if CharacterSet.lowercaseLetters.contains(scalars[0]), scalars.dropFirst().contains(where: { CharacterSet.uppercaseLetters.contains($0) }) {
            return true
        }
        // created_at, feature/auth
        for separator in ["_", "/"] as [Character] where core.contains(separator) {
            let parts = core.split(separator: separator, omittingEmptySubsequences: false)
            if parts.count > 1, parts.allSatisfy({ part in
                !part.isEmpty && part.unicodeScalars.allSatisfy { CharacterSet.alphanumerics.contains($0) }
            }) {
                return true
            }
        }
...
extension AppSettings.RecordKey {
    /// Short name for key caps and titles.
    var title: String {
        switch self {
        case .fn: "fn"
